#ifndef _LittleFSINIFILE_H
#define _LittleFSINIFILE_H

#define LittleFSINIFILE_VERSION "1.0.0"

// Maximum length for filename, excluding NULL char
#define LittleFSINI_FILE_MAX_FILENAME_LEN 31

#include "LittleFS.h"
#include "IPAddress.h"

class LittleFSIniFileState;

class LittleFSIniFile {
public:
	enum error_t {
		errorNoError = 0,
		errorFileNotFound,
	...
	// Create an LittleFSIniFile object. It isn't opened until open() is called on it.
	LittleFSIniFile(const char* filename, const char* mode = "r",
...
protected:
	...
	bool findKey(const char* section, const char* key, char* buffer,
				 size_t len, char** keyptr, LittleFSIniFileState &state) const;


private:
	char _filename[LittleFSINI_FILE_MAX_FILENAME_LEN];
	const char* _mode;
...
bool LittleFSIniFile::open(void)
{
	if (_file)
		_file.close();
...
void LittleFSIniFile::close(void)
{
	if (_file)
		_file.close();
}

bool LittleFSIniFile::isOpen(void) const
{
	return (_file == true);
}

LittleFSIniFile::error_t LittleFSIniFile::getError(void) const
{
	return _error;
}

void LittleFSIniFile::clearError(void) const
{
	_error = errorNoError;
}

const char* LittleFSIniFile::getMode(void) const
{
	return _mode;
}

const char* LittleFSIniFile::getFilename(void) const
{
	return _filename;
}
